...
console.log('########## 1. TOML co parse duoc khong ##########')
...
console.log('  identity:', cj.oauthAccount.emailAddress, '| userID:', cj.userID, '| numStartups giu:', cj.numStartups)
let s = JSON.parse(fs.readFileSync(path.join(FAKE, '.claude/settings.json'), 'utf8'))
console.log('  env da xoa het ANTHROPIC_*:', !s.env, '| model:', s.model, '| permissions:', s.permissions.allow.length)

console.log('\n  --- gia lap token refresh roi switch sang nickB ---')
...
console.log('  vault cua nickA da bat token refreshed:', kc.vaultRead(A) === refreshed)
cj = JSON.parse(fs.readFileSync(path.join(FAKE, '.claude.json'), 'utf8'))
console.log('  identity da doi sang:', cj.oauthAccount.emailAddress, '| userID:', cj.userID)

console.log('\n  --- switch nguoc lai nickA, phai dung token refreshed ---')
claude.apply(state, A, 'ts3')
store.save(state)
console.log('  keychain == refreshed (khong phai blobA cu):', kc.readSecret(kc.CLAUDE_SERVICE) === refreshed)

console.log('\n########## 3. peekExpiry ##########')
console.log('  nickA con han (ngay):', Math.round((kc.peekExpiry(kc.vaultRead(A)) - Date.now()) / 864e5))
console.log('  blob rac -> null:', kc.peekExpiry('not json') === null)

console.log('\n########## cleanup ##########')
kc.vaultDelete(A)
kc.vaultDelete(B)
kc.deleteSecret(kc.CLAUDE_SERVICE)
console.log('  vault + fake credential entry da xoa')
console.log('  real login khong bi cham (service that: "Claude Code-credentials")')
