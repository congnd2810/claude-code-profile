...
console.log('[model_providers.tuongtacfree] occurrences:', (toml.match(/\[model_providers\.tuongtacfree\]/g) ?? []).length)
console.log('service_tier still there:', /^service_tier/m.test(toml))
console.log('model_reasoning_effort still there:', /^model_reasoning_effort/m.test(toml))
console.log('personality still there:', /^personality/m.test(toml))
console.log('top-level model before first table:', toml.slice(0, toml.search(/^\[/m)).match(/^model.*$/gm))
...
console.log('keys after the provider table:', topKeysAfterProv)
...
console.log('permissions intact:', Array.isArray(s.permissions?.allow) ? `${s.permissions.allow.length} entries` : 'LOST')
const auth = JSON.parse(fs.readFileSync(path.join(FAKE, '.codex/auth.json'), 'utf8'))
console.log('codex auth_mode:', auth.auth_mode, '| tokens kept:', !!auth.tokens, '| key:', auth.OPENAI_API_KEY)
...
console.log('config.toml identical to previous apply:', toml2 === toml)
console.log('provider table occurrences after 2 applies:', (toml2.match(/\[model_providers\.tuongtacfree\]/g) ?? []).length)
